...
- **FE: ekran edycji aktywnej listy (PATCH /challenges/:id) — klient gotowy, UI do zrobienia** (Faza 2). Endpoint + kontrakt (`challengeUpdateSchema`, tri-state tytułu) oraz klient `updateChallenge` w `apps/web/src/lib/api.ts` istnieją; brakuje widoku/formularza edycji nagród i tytułu aktywnego wyzwania.
